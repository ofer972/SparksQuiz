...
import { WS_URL, joinSession } from "@/lib/api";
...
/** Shown when user lands on /play/[pin] without ?nickname= — they only enter nickname; PIN is from URL. */
function JoinWithNicknameOnly({ pin, onJoined }: { pin: string; onJoined: (nickname: string) => void }) {
  ...
  const handleJoin = async () => {
    ...
    if (!clean) return setError("Nickname is required");
    if (clean.length > 20) return setError("Nickname max 20 characters");
    setLoading(true);
    try {
      await joinSession(pin, clean);
      onJoined(clean);
    } catch (e: unknown) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-[#16213e] rounded-3xl p-8 shadow-xl space-y-4">
          <p className="text-gray-400 text-sm text-center">
            Game PIN: <span className="text-yellow-400 font-bold tracking-widest">{pin}</span>
          </p>
          <div>
...
function PlayerGame({ pin }: { pin: string }) {
  ...
  const [nicknameFromJoin, setNicknameFromJoin] = useState<string | null>(null);
  const nickname = searchParams.get("nickname") ?? nicknameFromJoin ?? "";
  ...
  // Landed on /play/[pin] without nickname → show nickname-only form (PIN is in URL)
  if (!nickname) {
    return (
      <JoinWithNicknameOnly
        pin={pin}
        onJoined={(n) => {
          setNicknameFromJoin(n);
          router.replace(`/play/${pin}?nickname=${encodeURIComponent(n)}`);
        }}
      />
    );
  }
